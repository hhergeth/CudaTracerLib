...
template<typename VolEstimator> struct PPPMPhotonParticleProcessHandler
{
	...
	CUDA_FUNC_IN void handleMediumInteraction(const Spectrum& weight, float accum_pdf, const Spectrum& last_f, MediumSamplingRecord& mRec, const NormalizedT<Vec3f>& wi, const TraceResult& r2, const VolumeRegion* bssrdf)
	{
		delta = false;
		bool storeVol = rng.randomFloat() < g_Parameters.probVolume;
