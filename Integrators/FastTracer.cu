...
CUDA_DEVICE Image g_DepthImage;
...
__global__ void doDirectKernel(unsigned int w, unsigned int h, k_PTDBuffer g_Intersector, Image I, float SCALE, bool depthImage)
{
	int rayidx;
	__shared__ volatile int nextRayArray[MaxBlockHeight];
	do
...
//CUDA_CONST Material* g_MAT;
template<bool NEXT_EVENT_EST> __global__ void pathIterateKernel(unsigned int N, Image I, int pass, int iterationIdx, bool depthImage)
{
...
void FastTracer::doDirect(Image* I)
{
...
void FastTracer::doPath(Image* I)
{
	bufA->Clear();
	cudaMemcpyToSymbol(g_NextRayCounterFT, &zero, sizeof(zero));
