...
public:
	...
	CUDA_DEVICE CUDA_HOST static float evalCubicInterp2D(const Vec2f &p, const float *values, const uint2 &size, const Vec2f &min, const Vec2f &max, bool extrapolate = false);
	CUDA_DEVICE CUDA_HOST static float evalCubicInterp2DN(const Vec2f &p, const float **nodes_, const float *values, const uint2 &size, bool extrapolate = false);
