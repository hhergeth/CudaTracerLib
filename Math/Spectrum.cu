...
{
	SpectrumHelper::staticData* d = SpectrumHelper::getData();
	float luminance = 0.0f;
	for (size_t i=0; i<SPECTRUM_SAMPLES; ++i)
		luminance += d->CIE_Y[i] * s[i];
	return luminance * d->CIE_normalization;
}
...
void Spectrum::fromLinearRGB(float r, float g, float b, Spectrum::EConversionIntent intent)
{
	...
	if (intent == EReflectance) {
		if (r <= g && r <= b) {
			...
			result += r * d->rgbRefl2SpecWhite;
			if (g <= b) {
				result += (g - r) * d->rgbRefl2SpecCyan;
				result += (b - g) * d->rgbRefl2SpecBlue;
			} else {
				result += (b - r) * d->rgbRefl2SpecCyan;
				result += (g - b) * d->rgbRefl2SpecGreen;
			}
		} else if (g <= r && g <= b) {
			// Compute reflectance spectrum with 'g' as minimum
			result += g * d->rgbRefl2SpecWhite;
			if (r <= b) {
				result += (r - g) * d->rgbRefl2SpecMagenta;
				result += (b - r) * d->rgbRefl2SpecBlue;
			} else {
				result += (b - g) * d->rgbRefl2SpecMagenta;
				result += (r - b) * d->rgbRefl2SpecRed;
			}
		} else {
			// Compute reflectance spectrum with 'b' as minimum
			result += b * d->rgbRefl2SpecWhite;
			if (r <= g) {
				result += (r - b) * d->rgbRefl2SpecYellow;
				result += (g - r) * d->rgbRefl2SpecGreen;
			} else {
				result += (g - b) * d->rgbRefl2SpecYellow;
				result += (r - g) * d->rgbRefl2SpecRed;
			}
		}
		result *= .94f;
		...
		if (r <= g && r <= b) {
			...
			result += r * d->rgbIllum2SpecWhite;
			if (g <= b) {
				result += (g - r) * d->rgbIllum2SpecCyan;
				result += (b - g) * d->rgbIllum2SpecBlue;
			} else {
				result += (b - r) * d->rgbIllum2SpecCyan;
				result += (g - b) * d->rgbIllum2SpecGreen;
			}
		}
		else if (g <= r && g <= b) {
			// Compute illuminant spectrum with 'g' as minimum
			result += g * d->rgbIllum2SpecWhite;
			if (r <= b) {
				result += (r - g) * d->rgbIllum2SpecMagenta;
				result += (b - r) * d->rgbIllum2SpecBlue;
			} else {
				result += (b - g) * d->rgbIllum2SpecMagenta;
				result += (r - b) * d->rgbIllum2SpecRed;
			}
		} else {
			// Compute illuminant spectrum with 'b' as minimum
			result += b * d->rgbIllum2SpecWhite;
			if (r <= g) {
				result += (r - b) * d->rgbIllum2SpecYellow;
				result += (g - r) * d->rgbIllum2SpecGreen;
			} else {
				result += (g - b) * d->rgbIllum2SpecYellow;
				result += (r - g) * d->rgbIllum2SpecRed;
			}
		}
		result *= .86445f;
	}
...
void Spectrum::toXYZ(float &x, float &y, float &z) const
{
	...
	x *= d->CIE_normalization;
	y *= d->CIE_normalization;
	z *= d->CIE_normalization;
}
...
void Spectrum::fromIPT(float I, float P, float T, Spectrum::EConversionIntent intent)
{
...
CUDA_FUNC_IN float toSRGBComponent(float value) {
...
struct InterpolatedSpectrum
{
	...
	CUDA_FUNC_IN float operator()(float f) const
	{
		return eval(f);
	}
};

struct ProductSpectrum
{
	const InterpolatedSpectrum &m_spec1;
	const InterpolatedSpectrum &m_spec2;

	ProductSpectrum(const InterpolatedSpectrum &s1,
		const InterpolatedSpectrum &s2) : m_spec1(s1),
		m_spec2(s2) { }
...
void SpectrumHelper::staticData::init()
{
	...
	CIE_X.fromContinuousSpectrum(CIE_wavelengths, CIE_X_entries, CIE_samples);
	CIE_Y.fromContinuousSpectrum(CIE_wavelengths, CIE_Y_entries, CIE_samples);
