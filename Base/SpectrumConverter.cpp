...
float3 SpectrumConverter::operator()(char* c)
{
	FW::String s(c);
	s = FW::String("C:\\Users\\Hannes\\Documents\\Visual Studio 2010\\Projects\\Libs\\pbrt-v2\\scenes\\pbrt-scenes\\").append(s);
	vector<float> wls, v;
	vector<float> vals;
	ReadFloatFile(s.getPtr(), &vals);
	for (unsigned int j = 0; j < vals.size() / 2; ++j)
	{
        wls.push_back(vals[2*j]);
        v.push_back(vals[2*j+1]);
    }
    Spectrum s2 = Spectrum::FromSampled(&wls[0], &v[0], wls.size());
	float rgb[3];
	s2.ToRGB(rgb);
