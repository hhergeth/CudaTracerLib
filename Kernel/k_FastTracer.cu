...
__global__ void pathCreateKernel(unsigned int w, unsigned int h, k_RayBuffer<k_FastTracer::rayData, 2> g_Intersector)
{
	...
	traversalRay* ray = g_Intersector[0].InsertRay(rayidx);
...
void k_FastTracer::doDirect(e_Image* I)
{
	...
	pathCreateKernel<<< dim3((w*h)/(32*8)+1,1,1), dim3(32, 8, 1)>>>(w, h, *intersector);
	...
	intersector->IntersectBuffers<false>(w * h, m_pScene->getNodeCount() == 1);
