...
CUDA_ONLY_FUNC Spectrum loadPixel(int x, int y, Image& img)
{
...
CUDA_ONLY_FUNC float d2(int x1, int y1, int x2, int y2, Image& img, SamplerpixelData* lumData)
{
	SamplerpixelData& e1 = lumData[y1 * img.getWidth() + x1], &e2 = lumData[y2 * img.getWidth() + x2];
...
CUDA_ONLY_FUNC float D2(int x1, int y1, int x2, int y2, int f, Image& img, SamplerpixelData* lumData)
{
	float sum = 0.0f;
...
CUDA_ONLY_FUNC float w(int x1, int y1, int x2, int y2, int f, Image& img, SamplerpixelData* lumData)
{
...
CUDA_GLOBAL void evalPass(Image img, SamplerpixelData* lumData, float* blockData, int nx, unsigned int* deviceNumSamples, unsigned int* contribPixels, float* deviceWeight)
{
...
void BlockSampler::Clear()
{
	m_uPassesDone = 0;
	hasValidData = false;
	ThrowCudaErrors(cudaMemset(m_pDeviceBlockData, 0, sizeof(float) * totalNumBlocks()));
	ThrowCudaErrors(cudaMemset(m_pLumData, 0, img->getWidth() * img->getHeight() * sizeof(SamplerpixelData)));
	ThrowCudaErrors(cudaMemset(m_pDeviceContribPixels, 0, sizeof(unsigned int) * totalNumBlocks()));
...
void BlockSampler::DrawVariance(bool blocks) const
{
