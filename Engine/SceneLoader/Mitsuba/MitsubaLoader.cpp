...
{
	ParserState S(scene, std::filesystem::path(scene_file).parent_path().string(), assume_rotated_coords, create_exterior_bssrdf, create_interior_bssrdf);
	for (auto ent : cmd_def_storage)
		S.def_storage.add(ent.first, ent.second);

	pugi::xml_document doc;
	if (!doc.load_file(scene_file.c_str()))
		throw std::runtime_error("couldn't loader scene xml!");
	CudaTracerLib::XMLNode root("", doc);
