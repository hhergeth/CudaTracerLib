...
CUDA_GLOBAL void rtm_SumLogLum(Image::Pixel* P, unsigned int w, unsigned int h, float splatScale)
{
...
template<typename TARGET> CUDA_GLOBAL void rtm_Scale(Image::Pixel* P, TARGET T, unsigned int w, unsigned int h, float splatScale, float L_w, float alpha, float L_white2, Filter filter, float scale)
{
...
template<typename TARGET> CUDA_GLOBAL void rtm_Copy(Image::Pixel* P, TARGET T, unsigned int w, unsigned int h, float splatScale, Filter filter, float scale)
{
	unsigned int x = threadIdx.x + blockDim.x * blockIdx.x, y = threadIdx.y + blockDim.y * blockIdx.y;
	if(x < w && y < h)
	{
...
#define BLOCK_HGT 8
template<typename TARGET> CUDA_GLOBAL void rtm_CopyShared(Image::Pixel* P, TARGET T, int w, int h, float splatScale, Filter filter, float scale)
{
...
#define LOAD_BLOCK(local_x, local_y) { \
	...
	int x0 = math::Ceil2Int(px.x - 0.5f - filter.As<FilterBase>()->xWidth);
	int x1 = math::Floor2Int(px.x + 1 - 0.5f + filter.As<FilterBase>()->xWidth);
	int y0 = math::Ceil2Int(px.y - 0.5f - filter.As<FilterBase>()->yWidth);
	int y1 = math::Floor2Int(px.y + 1 - 0.5f + filter.As<FilterBase>()->yWidth);
	x0 = max(x0, 0);
	x1 = min(x1, 0 + int(w) - 1);
	y0 = max(y0, 0);
	y1 = min(y1, 0 + int(h) - 1);
...
void Image::ComputeDiff(const Image& A, const Image& B, Image& dest, float scale)
{
	memTarget T1;
	texTarget T2;
...
template<typename TARGET> CUDA_GLOBAL void rtm_NumSamples(Image::Pixel* P, TARGET T, unsigned int w, unsigned int h, float numPasses)
{
	unsigned int x = threadIdx.x + blockDim.x * blockIdx.x, y = threadIdx.y + blockDim.y * blockIdx.y;
	if (x < w && y < h)
	{
...
void Image::DrawSamplePlacement(int numPasses)
{
	int block = 32;
	memTarget T1;
	texTarget T2;
