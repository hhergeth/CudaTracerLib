...
public:
	...
	bool Update()
	{
		...
		float3 pos = Camera->Position() + m_mView.TransformDirection(Velocity);
		...
		if((GetKeyState(VK_RBUTTON) & 0x80) != 0)
		{
			...
			else
			{
				...
				m_mView = float4x4::RotationAxis(m_mView.Right(), AngleAddY) % m_mView;
				m_mView = float4x4::RotationAxis(m_mView.Up(), AngleAddX) %  m_mView;
			}
		}

		float3 f = m_mView.Forward(), r = normalize(cross(f, make_float3(0, 1, 0))), u = normalize(cross(r, f));
		m_mView.col(0, make_float4(r.x, r.y, r.z, 0.0f));
		m_mView.col(1, make_float4(u.x, u.y, u.z, 0.0f));
		m_mView.col(2, make_float4(f.x, f.y, f.z, 0.0f));
