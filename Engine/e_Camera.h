...
struct e_CameraData
{
	...
	CUDA_ONLY_FUNC Ray GenRay(int2 pos, int2 size)
	{
		return GenRay(pos.x, pos.y, size.x, size.y);
	}
	CUDA_ONLY_FUNC Ray GenRay(float x, float y, float w, float h)
	{
	...
	CUDA_ONLY_FUNC Ray GenRay(int2 pos, int2 size, float u, float v)
	{
		return GenRay(pos.x, pos.y, size.x, size.y, u, v);
	}
	CUDA_ONLY_FUNC Ray GenRay(float x, float y, float w, float h, float u, float v)
	{
		float4 a = make_float4(2.0f * ((float)x / (float)w) - 1.0f, -(2.0f * ((float)y / (float)h) - 1.0f), 0, 1.0f);
		a = proj * a;
		a /= a.w;
...
public:
	...
	virtual void getData(e_CameraData& c) = 0;
	virtual float4x4 getViewProjection() = 0;
