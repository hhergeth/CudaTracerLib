...
int test(CudaRNG& rng, const HashGrid_Reg& grid, const AABB& box, Ray& r, float& rad, float& ttEnd)
{
	//r.origin = Vec3f(88.116699, -67.444046, 99.186798);
	//r.direction = Vec3f(-0.511254, 0.556284, -0.655109);
	r = Ray(Vec3f(-100), Vec3f(0.9f,1,1.1f).normalized());
...
void testGrid()
{
	...
	HashGrid_Reg grid(AABB(Vec3f(-100), Vec3f(100)), 0, 100 * 100 * 100);
