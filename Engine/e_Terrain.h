...
struct e_TerrainData_Inner
{
	...
	Vec2f calcVerticalRange()
	{
		Vec2f r = Vec2f(FLT_MAX, -FLT_MAX);
		for(int i = 0; i < 4; i++)
		{
			Vec2f q = blocks[i].ToFloat2();
			r.x = min(r.x, q.x);
			r.y = max(r.y, q.y);
		}
		return r;
...
struct e_TerrainData_Leaf
{
	...
	{/*
		...
		return rr;*/
		Vec4i l = Vec4i(data.x, data.y, data.z, data.w);
		...
		float H11 = M, H00 = H11 + half(o0), H02 = H11 + half(o1), H20 = H11 + half(o2), H22 = H11 + half(o3),
			  H01 = math::lerp(H00, H02, float(s0) / 255.0f), H21 = math::lerp(H20, H22, float(s1) / 255.0f),
			  H10 = math::lerp(H00, H20, float(s2) / 255.0f), H12 = math::lerp(H02, H22, float(s3) / 255.0f);
		float mi = min(min(H00, H01, H02), min(H10, H11, H12), min(H20, H21, H22)), ma = max(max(H00, H01, H02), max(H10, H11, H12), max(H20, H21, H22));
		return Vec2f(mi, ma);
	}

	CUDA_HOST Vec3f calcNormal(float dx, float dy)
	{
		...
		Vec3f n = -cross(Vec3f(dx * 2.0f, f02 - f00, 0), Vec3f(0, f20 - f00, dy * 2.0f));
		return normalize(n);
	}
	...
	{/*
#define C(B, V) signed char(math::clamp((V - B) / 8.0f, -128.0f, 127.0f))
		H0 = a0;
		off01 = C(a0, a1);
		off02 = C(a1, a2);
...
#undef C*/
		...
		offld = half(c0 - b1);
		offrd = half(c2 - b1);
#define INV(des, start, end) unsigned char(math::clamp((des - start) / (end - start) * 255.0f, 0.0f, 255.0f))
		offmu = INV(a1, a0, a2);
		offmd = INV(c1, c0, c2);
		offlm = INV(b0, a0, c0);
		offrm = INV(b2, a2, c2);
...
public:
	e_TerrainDataTransporter(Vec2i _off)
	{
		off = make_int2(0, 0);
		m_sOffset = _off;
	}
	...
	void operator()(unsigned int _x, unsigned int _y, Vec4f f)
	{
		*acc(_x, _y) = f;
	}
	void pushOffset(Vec2i o)
	{
		changes.push_back(o);
		off += o;
	}
	void popOffset()
	{
		Vec2i o = changes[changes.size() - 1];
		changes.pop_back();
		off -= o;
	}
};
...
public:
	e_Terrain(int a_Depth, Vec2f m, Vec2f M)
	{
	...
	e_KernelTerrainData getKernelData(bool devicePointer = true)
	{
		...
		q.m_sMax = Vec3f(m_sMax.x, m_sSpan.y, m_sMax.y);
		q.m_sMin = Vec3f(m_sMin.x, m_sSpan.x, m_sMin.y);
		q.m_uDepth = m_uDepth;
		return q;
	}
